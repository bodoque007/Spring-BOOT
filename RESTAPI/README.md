...
5. Run the application using Maven:
   ```bash
   mvn spring-boot:run
6. Go to http://localhost:8080/employees by default, if you haven't changed the app's port. For HTTP Verbs, the api follows REST principles (for example, to get the employee with ID 1, make a get request to localhost:8080/employee/1. 
To delete or put an employee with id "employee_id", do the corresponding request to localhost:8080/employees/{employee_id}. Remember to include the employee in JSON format if you're updating, just including the firstName, lastName and email properties). It requires external software as Postman to post, put or delete. Thus, the version in crudMVCDemo is much better, as it's instead an MVC CRUD web app that implements a UI to add, delete or update employees.